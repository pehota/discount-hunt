 ...
 * Submits the feed's full selection (post-SLICE contract: Generate builds from
 * EXACTLY the submitted items); checkboxes are UNCHECKED by default, so the helper
 * extracts every itemIds value from GET / — simulating a user who checks all shown
 * boxes (the feed is restriction-filtered).
 */
