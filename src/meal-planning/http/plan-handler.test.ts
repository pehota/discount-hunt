...
describe("PlanHandler", () => {
  ...
  test("empty selection → inline 200, NO persisted plan, existing plan preserved", async () => {
    ...
    await discountService.registerDiscountItem(
      { externalId: "i1", store: "Aldi", name: "Zucchini", category: "veg", regularPrice: 199, salePrice: 99, validUntil: vu, dietaryTags: [] },
      "job-001",
    );
  ...
  test("generate-from-selection happy path: only selected items counted", async () => {
    ...
    await discountService.registerDiscountItem(
      { externalId: "i2", store: "Aldi", name: "Rote Linsen", category: "legume", regularPrice: 249, salePrice: 149, validUntil: vu, dietaryTags: [] },
      "job-001",
    );
    await discountService.registerDiscountItem(
      { externalId: "i3", store: "Aldi", name: "Spinat", category: "veg", regularPrice: 179, salePrice: 89, validUntil: vu, dietaryTags: [] },
      "job-001",
    );
