...
- **Verified instead (non-destructive, hermetic, real code paths):** (a) drove `DiscountService.replaceStoreItems` on a real-DB COPY → new rows already-categorised (0 NULL) + 32 prior Aldi offers archived to `offer_history` (005A). (b) **Production wiring proven:** ran the real `runLiveScrape` (no injected `runScrape` → real `makeProdRunScrape`→`buildInfrastructure`→`resolveLlm`→`LlmCategoryClassifier`→`ScrapingService(…,classifier)`) with fake fetchers + `LLM_PROVIDER=claude-cli`, NO post-scrape hook, throwaway DB → Aldi fixture rows landed categorised (Produce / Meat & Fish), **0 NULL taxonomy**. So the full 004 chain (resolveLlm→classifier→classify-before-insert) is confirmed. **Only the live-data browser screenshot is outstanding** (nice-to-have; the behavior + wiring are proven).
